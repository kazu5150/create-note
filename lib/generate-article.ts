  ...
  "sections": [
    {
      ...
      "imagePrompt": "このセクションの内容をインフォグラフィックで表現する場合の構成要素（例：「AIと人間の比較を示す2パネル構成。左にAIアイコンと評価グラフ、右に人間アイコンと信頼性ゲージ。番号付き3項目リストと矢印で流れを表示」）"
    }
  ],
      ...
      raw.sections.map(async (s) => {
        if (s.imagePrompt) {
          try {
            const styledPrompt =
              s.imagePrompt +
              "。日本語の編集系インフォグラフィック。ダークネイビー（#1a2744）の背景。複数のパネルに分割し、フラットアイコン・番号付きリスト・矢印・ゲージメーター・棒グラフなどのデータビジュアライゼーション要素で構成。オレンジ・シアン・パープルのカラーアクセント。太字の白テキストラベル付き。プロフェッショナルで洗練されたレイアウト。人物の顔・写真なし（シンプルなアイコンのみ可）。横長16:9構図。";
