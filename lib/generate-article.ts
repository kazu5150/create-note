...
import { generateSvg, DiagramSpec } from "./generate-svg";
  ...
  "sections": [
    {
      ...
      "diagram": {
        "type": "flowchart" | "concept-map" | "comparison",
        "nodes": [{"id": "a", "label": "ラベル"}],
        "edges": [{"from": "a", "to": "b", "label": "任意のラベル"}]
      }
    }
  ],
  "conclusion": "まとめ（stop-ai-slop-jpルールを適用）"
}
diagramは省略可能。含める場合はそのセクションの内容を図解するものにする。`;
  ...
  const sections: Section[] = raw.sections.map((s) => {
    if (s.diagram) {
      return { ...s, svg: generateSvg(s.diagram) };
    }
    return s;
  });
